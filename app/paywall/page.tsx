...
const UNIVERSAL_LINK = 'https://app.respirelyf.com/paywall'

export default function PaywallPage() {

  const [copied, setCopied] = useState(false)

  const copyAndExplain = () => {
    if (typeof navigator !== 'undefined' && navigator.clipboard) {
      navigator.clipboard.writeText(UNIVERSAL_LINK)
      setCopied(true)
    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-4" style={{ background: 'radial-gradient(circle at center, rgba(80, 167, 226, 0.08) 0%, rgba(255, 255, 255, 1) 75%)' }}>
      <div className="max-w-md w-full text-center space-y-6">
        ...
        <p className="text-gray-600">
          Have the app? Copy the link below, paste it into Notes (or Messages), then tap the link there to open the subscription screen. Don&apos;t have the app? Download it from the App Store.
        </p>

        <button
          type="button"
          onClick={copyAndExplain}

    ...
    <section className="relative min-h-screen flex flex-col items-center overflow-hidden" style={{ background: 'radial-gradient(circle at center, rgba(80, 167, 226, 0.08) 0%, rgba(255, 255, 255, 1) 75%)' }}>
      ...
      <header className="w-full max-w-7xl mx-auto px-6 py-8 flex items-center justify-between relative z-30">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-xl font-extrabold tracking-tight text-charcoal font-display uppercase">Respire LYF</h2>
        </div>
      ...
      <AnimatePresence mode="wait">
        {!isLastQuestion ? (
          ...
          >
            ...
            {/* Right Column */}
            <motion.div
              className="w-full lg:w-1/2 h-full min-h-[400px] lg:min-h-[600px] relative"
              initial={{ opacity: 0, x: 50 }}
              animate={{ opacity: 1, x: 0 }}
              transition={{ duration: 0.8, delay: 0.3 }}
            >
              <div className="absolute inset-0 rounded-3xl overflow-hidden shadow-2xl shadow-primary/10 group" style={{ background: 'linear-gradient(135deg, #10b981 0%, #059669 100%)' }}>
                {/* Circle Icons Image */}
                ...
                {/* Live Analysis Card */}
                <motion.div
                  className="absolute bottom-8 right-8 p-6 rounded-2xl max-w-xs shadow-xl"
                  style={{
                    background: 'rgba(255, 255, 255, 0.7)',
                    backdropFilter: 'blur(12px)',
                    border: '1px solid rgba(255, 255, 255, 0.3)'
                ...
                >
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    ></motion.span>
                    <span className="text-[10px] uppercase tracking-widest font-bold text-charcoal/60">Live Analysis</span>
                  </div>
                  <p className="text-sm font-semibold text-charcoal leading-snug">Visualizing clinical outcomes through integrated HD/HI data points.</p>
                </motion.div>
              </div>
      ...
      <footer className="w-full px-6 py-8 flex flex-col items-center gap-6 relative z-20">
        ...
        <div className="flex flex-col items-center gap-2 opacity-60 hover:opacity-100 transition-opacity cursor-pointer group">
          <p className="text-[10px] tracking-[0.4em] font-bold uppercase text-charcoal/60">
            {isLastQuestion ? 'Final Sequence Complete' : 'Scroll to Discover'}
          </p>
          <motion.span
          ...
          >
            {isLastQuestion ? 'keyboard_double_arrow_down' : 'expand_more'}
          </motion.span>
        </div>
      ...
      {/* Floating Button */}
      <button className="fixed bottom-12 right-12 size-14 flex items-center justify-center rounded-full border border-charcoal/5 bg-white/80 backdrop-blur-xl shadow-lg shadow-black/5 hover:bg-white transition-all z-50 group">
        <span className="material-symbols-outlined text-charcoal/40 group-hover:text-primary transition-colors text-2xl">graphic_eq</span>
      </button>
